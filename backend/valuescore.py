...
_PRICE_LOG_SCALE = 140  # smaller = harder to reach a high price score; see price_component()
# Calibrated so a listing ~30% below the comparable-comps median price
# clamps price_component to (essentially) 100 - the overall score still
# won't hit a literal 100 unless location is also perfect, since it's a
# geometric mean of the two, but a genuinely steep discount now maxes out
# its half. This also makes the symmetric case - a listing priced ~30%
# ABOVE median - clamp toward 0, which is intentional.
